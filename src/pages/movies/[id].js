  ...
  return (
    <>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Container sx={{ display: 'flex', flexDirection: 'column', alignItems: 'start', justifyContent: 'start', gap: '20px' }}>
          ...
          <Container sx={{ ml: -2, display: 'flex', flexDirection: 'row', gap: '20px', mt: "50px", alignItems: 'start', justifyContent: 'start' }}>
            {movie.genres.length !== 0 && (
              <Typography
                sx={{ ml: 2, lineHeight: 1, fontWeight: 900, fontSize: '1rem' }}
              >
                Movie genres:
              </Typography>
            )}
