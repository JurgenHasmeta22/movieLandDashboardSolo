    ...
    async function getDataPaginated() {
        const res = await axios.get(
            `http://localhost:4000/${tableName}/page/${page + 1}?perPage=${pageSize}${
                sortModelField && sortModelField !== undefined && sortModelField !== null
                    ? `&sortBy=${sortModelField}`
                    : ""
            }${
                sortModelOrder && sortModelOrder !== undefined && sortModelOrder !== null
                    ? `&ascOrDesc=${sortModelOrder}`
                    : ""
            }${
                searchedTerm &&
